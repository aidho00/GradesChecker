...
Web/desktop Flutter + PHP/Database checker with simple access login for comparing legacy UNO promotional-list Excel files against current SMS grade records.
...
- Validates the uploaded Excel as a UNO promotional-list style file before accepting it.
...
- Shows the Excel subject description inside the subject cell and the subject inspection modal.
- Ranks same-code `tbl_subject` choices using Excel subject description and units for better subject comparison.
...
- Profile creation includes editable student details, course selector from `tbl_course`, year level, gender, and birthdate.
